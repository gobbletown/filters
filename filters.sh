...
cat-urls.sh
filter-urls.sh | uniqnosort
xurls | uniqnosort                                                                  # filter urls
org clink
get-1-grams.sh
...
scrape '[a-zA-Z0-9_]+' | sed '/^$/d'                                                # word chars alphanumeric underscore
scrape '[a-zA-Z0-9-]+' | sed '/^$/d'                                                # word chars alphanumeric dash
word-chars-alphanumeric-dash-underscore-dash.sh
scrape '[0-9.]+' | sed '/^$/d'                                                      # float
...
erase-starting-whitespace
erase-trailing-whitespace
remove-starting-and-trailing-whitespace.sh                                          # erase leading and trailing whitespace
sed '/\/\.git\//d'                                                                  # no git
delete-up-to.sh ,
delete-up-to.sh |
delete-up-to.sh ;
...
scrape '[a-zA-Z0-9-_]+' | sed '/^$/d' | filter-rtc-issue-ids.sh
scrape '[a-zA-Z0-9-_]+' | sed '/^$/d' | filter-nbo-issue-ids.sh
scrape '[a-zA-Z0-9-_]+' | sed '/^$/d' | filter-jira-issue-ids.sh
...
f-efw | remove-leading-and-trailing-newlines.sh                                     # remove all leading and trailing starting and ending whitespace
...
scrape '(?<=")[\w]+'                                                                # Filter the selection by selecting only uppercase words that come directly after a double quote.
...
scrape-hex-24
scrape '"([^"]+)"' | uq                                                             # Scrape literal
scrape-string "'"                                                                   # Scrape single quoted strings
scrape-string '"'                                                                   # Scrape double quoted strings
...
scrape "[a-zA-Z_-]+: ?[^ ]+"                                                        # key: value pairs                                                                                           # for quickly scraping logs
scrape "\x23([0-9A-Za-z]{6}|[0-9A-Za-z]{3})\b"                                      # hex
